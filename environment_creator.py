...
def create_env_multiagent_five_small_obs_continuous(initial_pos, goal):
    ...
    obs = [
        RobotArenaState(
            np.pad(ob, (0, 2), "constant"), goal=None, obstacles=None, radius=c.obs_size
        )
        for ob in obstacles_positions
    ]
    dynamic_obs = obs[-1]
    ...
    real_env_1 = BetterRobotArena(
        initial_state=initial_state, gradient=True, discrete=False, sim=False, config=c
    )
    sim_env_1 = BetterRobotArena(
        initial_state=initial_state, gradient=True, discrete=False, sim=True, config=c
    )
    real_env_2 = BetterRobotArena(
        initial_state=dynamic_obs, gradient=True, discrete=False, sim=False, config=c
    )
    sim_env_2 = BetterRobotArena(
        initial_state=dynamic_obs, gradient=True, discrete=False, sim=True, config=c
    )
